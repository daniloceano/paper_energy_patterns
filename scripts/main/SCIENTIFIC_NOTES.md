...
## 10. Figure S2 — Selected EP1 and EP2 tracks

**Scientific purpose:** To document which cyclones are used in the composite
dynamical analysis (Figure 7) and show their geographic distribution.

The figure shows **both** EP1 (purple) and EP2 (dodger blue) cyclones with
distinct visual encoding:
- Complete track (gray, thin): full lifecycle for context
- Intensification phase (colored, thick): the temporal window used for compositing
- Genesis marker (symbol at track start): EP1 = circle, EP2 = square

**What this figure establishes:**
1. The number of cases used (varies depending on data coverage from the Zenodo archive)
2. That the selected cases span the full range of South Atlantic genesis latitudes
3. That intensification segments are broadly distributed, not restricted to a single
   geographic corridor, so the composite is not dominated by a geographically narrow
   sub-selection

**Caveats:**
- Cases with missing or incomplete LEC data from the Zenodo archive are excluded.
  The effective sample sizes (EP1 and EP2 used) are printed to the terminal at runtime
  and shown in the figure legend.
- The figure uses `results/ep_structure/ep{1,2}_cases.csv`, which is generated by
  `step1_select_ep_tracks.py`. If selection criteria change, these files must be
  regenerated before re-running this script.

---

## 11. Figure S3 — Vertical Ca/Ck profiles (EP1)

**Scientific purpose:** To determine which pressure level(s) carry the strongest
baroclinic ($C_a$) and barotropic ($C_k$) energy conversion signals in EP1 cyclones,
and thereby justify the pressure-level choice used in the instability composites.

### 11.1 Vertical profiles

For each EP1 cyclone with available Zenodo LEC data, the time-mean (over
intensification phase) profiles of $C_a(p)$ and $C_k(p)$ at 32 pressure levels
(1000–100 hPa) are computed and collected into distributions across the EP1
population. The figure shows boxplots of these distributions, revealing:
...
- **Peak $C_a$** level: typically in the mid-troposphere (~350–400 hPa), near the
  ...
  gradient and the upper-level jet.
- **Minimum $C_k$** level: typically also near 350 hPa, indicating that the
  barotropic response (eddy → mean KE transfer, i.e., $C_k < 0$ = barotropic
  damping) is strongest in the same layer.

### 11.2 Data corrections
...
vertical integral of `Ck_level.csv` with integrated `Ck` in the same ensemble (see
`scripts/ep_structure_analysis/audit_storm_centering.py` or equivalent validation).
...
> re-validate whether these corrections still apply before using S3.
...
suggests that in the same layer the cyclone is simultaneously extracting energy
baroclinically ($C_a > 0 \Rightarrow$ APE from mean temperature gradient) and
...
## 12. Global caveats and interpretation boundaries
