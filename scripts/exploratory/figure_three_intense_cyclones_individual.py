...
def plot_lps_conversion(track_id, energy_data, output_dir):
    ...
    # Add title
    lps.ax.set_title(f'Cyclone {track_id} - Conversion Phase Space', 
                    fontsize=12, fontweight='bold', pad=10)
    
    # Save
    output_file = output_dir / f'{track_id}_lps_conversion.png'
...
def plot_lps_imports(track_id, energy_data, output_dir):
    ...
    # Add title
    lps.ax.set_title(f'Cyclone {track_id} - Imports Phase Space', 
                    fontsize=12, fontweight='bold', pad=10)
    
    # Save
    output_file = output_dir / f'{track_id}_lps_imports.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(lps.fig)
...
def plot_track_map(track_id, track, output_dir):
    ...
    ax.legend(handles=all_handles,
              title='Legend (Ke in 10$^5$ J m$^{-2}$)',
              loc='lower left', framealpha=0.95, edgecolor='black', fontsize=9, title_fontsize=9,
