...
def export_predep() -> list[dict]:
    ...
    # Per-EP standalone files (ep1, ep2, ep3, epall — generated by --ep N flag)
    import re as _re
    for ftype in ("absolute", "anomaly"):
        for ep_file in sorted(RESULTS.glob(f"step7_predep_{ftype}_ep*.csv")):
            # match _ep1, _ep2, _ep3 but also _epall
            if _re.search(r"_ep\d+\.csv$|_epall\.csv$", ep_file.name):
