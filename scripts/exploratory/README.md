...
| `ca_ck_ge_start_vs_end_distributions.py` | 3×2 KDE comparison of Ca/Ck/Ge at the first vs last valid timestep of the intensification and mature phases, with paired Wilcoxon tests — checks how much the phase mean (used in the LPS diagrams) smooths over within-phase evolution |
...
- `data/tracks_SAt_filtered_with_energetics_processed.csv` + `load_tracks()` phase labels — 3-hourly Ca/Ck/Ge merged with 1-hourly phase labels (for `ca_ck_ge_start_vs_end_distributions.py`)
